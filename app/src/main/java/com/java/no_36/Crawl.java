...
public class Crawl {
    ...
    // zhm added: new way of using get_picture
    // 传入一些keyword，据此进行搜索
    public static String[] get_picture(Keyword[] keywords)
    {
        String[] pictures = null;
        if (keywords.length >= 3)
        {
            String x = keywords[0].word + "+" + keywords[1].word + "+" + keywords[2].word;
            pictures = get_picture(x);
            if (pictures != null)
                return pictures;
        }
        if (keywords.length >= 2)
        {
            String x = keywords[0].word + "+" + keywords[1].word;
            pictures = get_picture(x);
            if (pictures != null)
                return pictures;
        }
